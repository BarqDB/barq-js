...
1. **Generate the binding** from barq-core's `bindgen/spec.yml` and the SDK
   ...
   ```sh
   npm run bindgen:wrapper -w @barq/barq   # -> src/binding/wrapper.generated.ts
   npm run bindgen:jsi     -w @barq/barq   # -> binding/jsi/jsi_init.cpp
   ```
2. **Compile the native module** (node) linking barq-core:
   ```sh
   npm run build:node -w @barq/barq        # cmake-js -> prebuilds/node/barq.node
   ```
3. **Build the TypeScript layer**:
   ```sh
   npm run build:ts -w @barq/barq
   ```
...
The [Release workflow](.github/workflows/release.yml) publishes the `@barq/*`
