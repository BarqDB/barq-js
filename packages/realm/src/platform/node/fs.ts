...
inject({
  ...
  setDefaultDirectoryPath(path) {
    ...
    return binding.JsPlatformHelpers.setDefaultBarqFileDirectory(path);
  },
  getDefaultDirectoryPath() {
    return binding.JsPlatformHelpers.defaultBarqFileDirectory();
  },
  exists(path) {
    debug("exists", path);
  ...
  copyBundledBarqFiles() {
    throw new Error("Barq for Node does not support this method.");
  },
  /*
  readDirectory(path) {
  ...
  */
  removeBarqFilesFromDirectory(path: string) {
