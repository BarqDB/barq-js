...
import { createBarqContext } from "..";
import { randomBarqPath } from "./helpers";

class Item extends Barq.Object {
  id!: number;
  name!: string;
  tags!: Barq.List<Tag>;
...
const configuration: Barq.Configuration = {
  schema: [Item, Tag],
  inMemory: true,
  path: randomBarqPath(),
};
...
let testBarq: Barq = new Barq(configuration);

const { useQuery, useObject, BarqProvider, useBarq } = createBarqContext(configuration);
...
const UseObjectItemComponent: React.FC<{ item: Item | (Item & Barq.Object) }> = React.memo(({ item }) => {
...
const keyExtractor = (item: Item & Barq.Object) => `${item.id}`;
...
const TestComponent = ({ queryType, useUseObject }: { queryType: QueryType; useUseObject: boolean }) => {
  ...
  const renderItem = useCallback<ListRenderItem<Item & Barq.Object>>(
...
function getTestCollection(queryType: QueryType) {
  switch (queryType) {
    case QueryType.filtered:
      return testBarq.objects(Item).filtered(...FILTER_ARGS);
    case QueryType.sorted:
      return testBarq.objects(Item).sorted(...SORTED_ARGS);
    case QueryType.normal:
      return testBarq.objects(Item);
  }
}
...
`("useQueryRender: $queryTypeName", ({ queryType }) => {
  beforeEach(() => {
    testBarq = new Barq(configuration);
  });
  afterEach(() => {
  ...
  it("change to data will rerender", async () => {
    ...
    await act(async () => {
      forceSynchronousNotifications(testBarq);
    });
  ...
  it("collection objects rerender on changes to their linked objects", async () => {
    ...
    testBarq.write(() => {
      const tag1 = testBarq.create(Tag, { id: 1, name: "a123" });
      const tag2 = testBarq.create(Tag, { id: 2, name: "b234" });
      const tag3 = testBarq.create(Tag, { id: 3, name: "c567" });
    ...
    testBarq.write(() => {
      testBarq.delete(collection[2].tags[2]);
    });
  ...
  // This replicates the issue https://github.com/BarqDB/barq-js/issues/4375
  it("will handle multiple async transactions", async () => {
    ...
    const performTest = async () => {
      ...
      while (i < 10) {
        ...
        const id = i;
        testBarq.write(() => {
          return new Item(testBarq, { id, name: `${id}` });
        });
        await new Promise<void>((resolve) => setTimeout(resolve, 0));
        testBarq.write(() => {
          const item = testBarq.objectForPrimaryKey(Item, id);
          if (item) {
            item.name = `${id + 100}`;
          }
        });
