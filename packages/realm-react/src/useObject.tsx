...
type BarqClassType<T = any> = { new (...args: any): T };
...
export type UseObjectHook = {
  <T>(options: ObjectHookOptions<T>): (T & Barq.Object<T>) | null;
  <T extends AnyBarqObject>(options: ObjectHookClassBasedOptions<T>): T | null;

  <T>(type: string, primaryKey: T[keyof T], keyPaths?: string | string[]): (T & Barq.Object<T>) | null;
  <T extends AnyBarqObject>(type: BarqClassType<T>, primaryKey: T[keyof T], keyPaths?: string | string[]): T | null;
};

/**
 * Generates the `useObject` hook from a given `useBarq` hook.
 * @param useBarq - Hook that returns an open Barq instance
 * @returns useObject - Hook that is used to gain access to a single Barq object from a primary key
 */
export function createUseObject(useBarq: UseBarqHook): UseObjectHook {
  function useObject<T extends AnyBarqObject>(
    ...
    if (updatedRef.current) {
      ...
      objectRef.current = new Proxy(object, {}) as T & Barq.Object<T>;
  ...
  return function useObjectOverload<T extends AnyBarqObject>(
    typeOrOptions: string | BarqClassType<T> | ObjectHookOptions<T> | ObjectHookClassBasedOptions<T>,
    primaryKey?: T[keyof T],
    keyPaths?: string | string[],
  ): T | null {
