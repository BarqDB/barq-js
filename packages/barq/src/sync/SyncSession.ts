...
export function toBindingNotifyBeforeClientReset(onBefore: ClientResetBeforeCallback) {
  return (internal: binding.Barq) => {
    onBefore(new indirect.Barq(null, { internal }));
  };
}

/** @internal */
export function toBindingNotifyAfterClientReset(onAfter: ClientResetAfterCallback) {
  return (internal: binding.Barq, tsr: binding.ThreadSafeReference) => {
    onAfter(
      new indirect.Barq(null, { internal }),
...
export function toBindingNotifyAfterClientResetWithFallback(
  ...
  return (internal: binding.Barq, tsr: binding.ThreadSafeReference, didRecover: boolean) => {
    if (didRecover) {
      onAfter(
        new indirect.Barq(null, { internal }),
        new indirect.Barq(null, { internal: binding.Helpers.consumeThreadSafeReferenceToSharedBarq(tsr) }),
 ...
 */
const mockApp = new Proxy({} as unknown as User, {
  get() {
...
export class SyncSession {
  ...
  /**
   * Gets the Sync-part of the configuration that the corresponding Barq was constructed with.
   */
  get config(): SyncConfiguration {
    return this.withInternal((internal) => {
      ...
      const user = User.get(internal.user);
  ...
  /**
   * Gets the URL of the Barq Object Server that this session is connected to.
   */
  get url() {
    const url = this.withInternal((internal) => internal.fullBarqUrl);
    if (url) {
      return url;
    } else {
   ...
   * underlying connection to the Barq Object Server.
   ...
   * downloaded and applied to the Barq or the specified timeout is hit in which case it will be rejected. If the method
   * times out, the download will still continue in the background.
   *
   * This method cannot be called before the Barq has been opened.
