...
export enum SubscriptionSetState {
   ...
   * Barq. To view the actual error, use `Subscriptions.error`.
   *
   ...
   * and instead obtain a new instance from {@link Barq.subscriptions}.
   */
...
export abstract class BaseSubscriptionSet {
   ...
   *  e.g. `Barq.objects("Cat").filtered("age > 10")`.
   * @returns The subscription with the specified query, or `null` if the subscription is not found.
   */
  findByQuery<Subscription>(query: Results<Subscription & BarqObject<Subscription>>): Subscription | null {
