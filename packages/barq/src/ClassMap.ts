...
import type { CanonicalObjectSchema, Constructor, BarqObjectConstructor } from "./schema";
...
export class ClassMap {
  ...
  private static createClass<T extends BarqObjectConstructor = BarqObjectConstructor>(
    schema: binding.ObjectSchema,
    constructor: Constructor | undefined,
  ): T {
    const result = ClassMap.createNamedConstructor<T>(schema.name);
    // Make the new constructor extend BarqObject
    // TODO: Use the end-users constructor, instead of `BarqObject` if provided
    if (constructor) {
      Object.setPrototypeOf(result, constructor);
      Object.setPrototypeOf(result.prototype, constructor.prototype);
    } else {
      Object.setPrototypeOf(result, BarqObject);
      Object.setPrototypeOf(result.prototype, BarqObject.prototype);
    }
    return result;
  }
  ...
  private static defineProperties(
    ...
    for (const property of properties) {
      ...
      Object.defineProperty(constructor.prototype, propertyName, {
        ...
        set(this: BarqObject, value: unknown) {
    ...
    this.mapping = Object.fromEntries(
      realmSchema.map((objectSchema, index) => {
        ...
        setClassHelpers(constructor, {
          ...
          wrapObject(obj) {
            if (obj.isValid) {
              return BarqObject.createWrapper(obj, constructor);
            } else {
              return null;
            }
          },
  ...
  public get<T>(arg: string | binding.TableKey | BarqObject<T> | Constructor<BarqObject<T>>): Constructor<T> {
    if (typeof arg === "string") {
    ...
    } else if (arg instanceof BarqObject) {
      return this.get(arg.constructor.name) as Constructor<T>;
    } else if (typeof arg === "function") {
      assert.extends(arg, BarqObject);
      ...
      assert(
        ...
        "Constructor was not registered in the schema for this Barq",
      );
  ...
  public getHelpers<T>(arg: string | binding.TableKey | BarqObject<T> | Constructor<BarqObject<T>>) {
    const constructor = this.get(arg);
    return getClassHelpers(constructor as unknown as typeof BarqObject);
  }
}
