...
): asserts objectSchema is BarqObjectConstructor | ObjectSchema {
  try {
    // Schema is passed via a class based model (BarqObjectConstructor)
    if (typeof objectSchema === "function") {
      ...
      if (!(objectSchema.prototype instanceof indirect.Object)) {
        ...
        if (typeof schemaName === "string" && schemaName !== objectSchema.name) {
          throw new TypeError(
            `Class '${objectSchema.name}' (declaring '${schemaName}' schema) must extend Barq.Object`,
          );
        } else {
          throw new TypeError(`Class '${objectSchema.name}' must extend Barq.Object`);
        }
      }
