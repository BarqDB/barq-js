 ...
 * that can be stored as properties of Barq objects. It can be
  ...
  implements Omit<ReadonlyArray<T>, "entries">
{
  ...
  *entries(): Generator<EntryType> {
  ...
  indexOf(searchElement: T, fromIndex?: number): number {
    ...
    if (this.type === "object") {
      assert.instanceOf(searchElement, BarqObject);
  ...
  // TODO: Implement support for BarqObjects, by comparing their #objectKey values
  /**
   ...
   * @note This is currently only supported for collections of Barq Objects.
   * @example
   ...
   *
   * A collection of Barq Objects can be sorted on one or more properties of
   ...
   * filters applied.
   *
   * This is **not** a _deep_ snapshot. Barq objects contained in this
   * snapshot will continue to update as changes are made to them, and if
   * they are deleted from the Barq they will be replaced by `null` at the
   * respective indices.
