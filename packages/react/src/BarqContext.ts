...
import { createUseBarq } from "./useBarq";
import {
  DynamicBarqProvider,
...
export type BarqContext<BarqProvider = DynamicBarqProvider> = {
   ...
   * }
   * ```
   * @param props - The {@link Barq.Configuration} or {@link Barq} of the provider
...
/**
 * Creates Barq React hooks and Provider component for a given Barq configuration
 * @example
 * ```
 ...
 *  };
 * }
 * const {useBarq, useQuery, useObject, BarqProvider} = createBarqContext();
 * ...
 * <BarqProvider schema={[Task]}></BarqProvider>
 * ```
 * @example
 * ```
...
export function createBarqContext(
  ...
  const useProgress = createUseProgress(useBarq);
