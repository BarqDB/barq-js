...
type CachedObjectArgs = {
  /**
   * The {@link Barq.Object} to proxy
   */
  object: Barq.Object | null;
  /**
...
/**
 * Creates a proxy around a {@link Barq.Object} that will return a new reference
 * on any relevant update to the object itself. It also wraps accesses to {@link Barq.List}
 ...
 * @returns Proxy object wrapping the {@link Barq.Object}
 */
export function createCachedObject({
  ...
  // Create a cache for any Barq.List properties on the object
  ...
  // of type `Barq.List`, and returns a `cachedCollection` wrapping those properties
  // to allow changes in the list to trigger re-renders
  const cachedObjectHandler: ProxyHandler<Barq.Object> = {
    get: function (target, key, receiver) {
      const value = Reflect.get(target, key, receiver);

      // If its a Barq.List we need to add a proxy cache around it
      if (value instanceof Barq.List && value.type === "object") {
  ...
  const listenerCallback: Barq.ObjectChangeCallback<any> = (obj, changes) => {
    updatedRef.current = true;
    if (changes.deleted) {
      updateCallback();
    } else if (changes.changedProperties.length > 0) {
      // Don't force a second re-render if any of the changed properties is a Barq.List,
      ...
      const anyListPropertyModified = changes.changedProperties.some((property) => {
        return obj[property] instanceof Barq.List && obj[property].type === "object";
      });
