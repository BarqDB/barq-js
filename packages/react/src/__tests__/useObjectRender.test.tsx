...
const useObject = createUseObject(useBarq);
...
// see https://github.com/BarqDB/barq-js/issues/5185
...
const ItemList: React.FC<{ list: Barq.List<ListItem> }> = React.memo(({ list }) => {
...
async function setupTest() {
  ...
  testBarq.write(() => {
    testBarq.create(List, { id: parentObjectId, title: "List", items: testCollection });
  });

  const object = testBarq.objectForPrimaryKey(List, parentObjectId);
  if (!object) throw new Error("Object not found in Barq");
...
describe("useObject: rendering objects with a Barq.List property", () => {
  afterEach(() => {
  ...
  describe("rendering single object", () => {
    ...
    it("it nullifies the object when it is deleted", async () => {
      ...
      expect(testBarq.objectForPrimaryKey(List, parentObjectId)).toBe(null);
  ...
  describe("rendering objects with a Barq.List property", () => {
    ...
    it("re-renders the list even if the list items have not been rendered", async () => {
      ...
      const list = testBarq.write(() => {
        return testBarq.create(List, { id: parentObjectId, title: "List" });
      });
      ...
      testBarq.write(() => {
        list.items.push(testBarq.create(ListItem, testCollection[0]));
      });
      ...
      testBarq.write(() => {
        list.items.push(testBarq.create(ListItem, testCollection[1]));
      });
