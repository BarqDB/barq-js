...
type CachedCollectionArgs<T> = {
  /**
   * The {@link Barq.Collection} to proxy
   */
  collection: Barq.List<T> | Barq.Results<T>;
...
/**
 * Creates a proxy around a {@link Barq.Collection} that will create new {@link Barq.Object}
 ...
 *
 * This makes the {@link Barq.Collection} behaves in an immutable way, as React expects, so
 * that a {@link Barq.Object} can be wrapped in {@link React.memo} to prevent unnecessary
...
export function createCachedCollection<T extends Barq.Object<any>>({
  ...
  isDerived = false,
  keyPaths,
}: CachedCollectionArgs<T>): { collection: Barq.Results<T> | Barq.List<T>; tearDown: () => void } {
  const cachedCollectionHandler: ProxyHandler<Barq.Results<T> | Barq.List<T>> = {
  ...
  const listenerCallback: CollectionCallback = (listenerCollection, changes) => {
    if (changes.deletions.length > 0 || changes.insertions.length > 0 || changes.newModifications.length > 0) {
      ...
      // Blocking issue: https://github.com/BarqDB/barq-core/issues/5220
