...
<!-- * Using Barq Core vX.Y.Z -->
<!-- * Upgraded Barq Core from vX.Y.Z to vA.B.C -->
...
### Compatibility
* React Native >= v0.71.4
* See "Compatibility" for a specific Barq version in [Barq's CHANGELOG](https://github.com/BarqDB/barq-js/blob/main/CHANGELOG.md).
...
### Deprecations
* Deprecated calling `useQuery` with three positional arguments. ([#6360](https://github.com/BarqDB/barq-js/pull/6360))
...
### Fixed
* Improved type generation, which fixed an issue with the `Barq.User` return from `useUser` ([#6196](https://github.com/BarqDB/barq-js/issues/6196))
* `UserProvider` will now always return a new `user` reference on change events. ([#6186](https://github.com/BarqDB/barq-js/issues/6186))
* Fix `useQuery` re-render when updating the given `type`. ([#6235](https://github.com/BarqDB/barq-js/issues/6186))
...
### Fixed
* `useObject` will now re-render if the result of `objectForPrimaryKey` is `null` or `undefined`. ([#6101](https://github.com/BarqDB/barq-js/issues/6101))
...
* `useEmailPasswordAuth` was crashing on v11 since there are no named exports on `Barq`.
...
### Enhancements
* Improve AppProvider and UserProvider rendering performance ([#5215](https://github.com/BarqDB/barq-js/pull/5215))

### Fixed
* Stabilized references for Collections, Lists and Objects ([#5269](https://github.com/BarqDB/barq-js/issues/5269))
...
### Fixed
* Fix crash when using `useObject` with a model containing a `List` of primitives ([#5058](https://github.com/BarqDB/barq-js/issues/5058))
* Fix potential infinite rerender issue when using `useUser` and the `UserProvider` ([#4967](https://github.com/BarqDB/barq-js/issues/4967))
...
### Fixed
* Results from `useQuery` could not be passed to `MutableSubscriptionSet.add`/`remove` ([#4507](https://github.com/BarqDB/barq-js/issues/4507), since v0.1.0)
...
* Implicit children [was removed from `React.FC`](https://solverfox.dev/writing/no-implicit-children/). Children has now been explicitly added to provider props. ([#4565](https://github.com/BarqDB/barq-js/issues/4565))
* Fixed potential "Cannot create asynchronous query while in a write transaction" error with `useObject` due to adding event listeners while in a write transaction ([#4375](https://github.com/BarqDB/barq-js/issues/4375), since v0.1.0)
...
* Tests run with `--forceExit` to prevent them hanging ([#4531](https://github.com/BarqDB/barq-js/pull/4531))
...
* Fixed bug when trying to access a collection result with an out of bounds index.([#4416](https://github.com/BarqDB/barq-js/pull/4416), since v0.2.0)
...
* List properties of a Barq.Object now rerender on change
