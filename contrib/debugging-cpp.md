...
The [launch.json](https://github.com/BarqDB/barq-js/blob/main/.vscode/launch.json) file contains various useful debugger launch configurations which attach `lldb` to the `node` process so that breakpoints can be set and exceptions can be caught. These profiles can be selected from the list in the top of the `Run and Debug` pane in VS Code.

[//]: # "TODO: Possibly update this statement or launch.json (we only have 1 'preLaunchTask'):"
All the launch configurations are configured to compile Barq JS in debug mode (by prelaunching a task from [tasks.json](https://github.com/BarqDB/barq-js/blob/main/.vscode/tasks.json)) before starting the debug session, so you do not need to remember to compile between sessions.
...
## Debugging Barq C++ in an iOS app using Xcode
...
## Debugging Barq C++ in an Android app using Android Studio

To debug Barq C++ in an Android app using Android Studio (the integration test is already set up to do this so you shouldn't need to do it for that):

1. Ensure you are using a debug version of the Barq `.so` (`node scripts/build-android.js  --arch=x86 --build-type=Debug`)
2. Prevent Gradle from stripping debug symbols from Barq by adding to your `app/build.gradle` in the `android.buildTypes.debug` section:
   ```
...
You should now be able to navigate to Barq C++ source files and add breakpoints by navigating to the "Project Files" view (using the "Project" dropdown in the top left file browser).

### Debugging Barq C++ in an Android app using lldb
...
1. Open VS Code with the relevant source files (make sure you use the same directory which Barq JS was compiled in)
...
1. Add `template<typename T> struct TD;` somewhere at the top level of a header file before the code you are looking at (probably in the `.hpp` file you are working with, just above the method in question, for Barq JS)
...
Using a debug version of Barq (e.g. [compile Barq in debug mode](./building.md#building-barq-js-1), then `npm i ~/dev/barq-js` to install your local version into your Node project) may yield more useful information.
